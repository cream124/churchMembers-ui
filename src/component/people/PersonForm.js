...
import { Alert, Button, Checkbox, FormControl, FormControlLabel, FormGroup, Grid, IconButton, InputLabel, MenuItem, Paper, Select, Snackbar, Switch, Typography } from "@mui/material";
...
import { SavePersonsDB, UpdatePersonDB } from '../../api/SavePersonDB';
...
function getPersonData(registeredUser){
  let person = {
    name: '',
    lastName: '',
    motherLastName: '',
...
export default function PersonForm(props) {
  const {registeredUser, isNew, personData} = props;

  // const [body, setBody] = React.useState(getPersonData(registeredUser, id))
  
  const [body, setBody] = React.useState(personData);
  ...
  const savePerson = async (data) => {
    ...
    if (isNew){
      ...
      try {
        const response = await updatePersonDB.updatePerson({ variables: body })
  ...
  return (
    <Paper  elevation={24} className={classes.container}>
      ...
      <Button
        ...
        variant="outlined"
        color="success"
        onClick={handleSubmit(savePerson)}
      >
        Guardar
      </Button>
