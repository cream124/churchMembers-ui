...
function activePersonsColumns(action) {
  return [
    ...
    },
    {
      ...
      type: 'actions',
      width: 100,
      getActions: (params) => getActionButtons(params, action),
      headerClassName: 'super-app-theme--header'
    }
  ]
}
function getActionButtons(params, acction) {
  const actionButtons = {
    print: <GridActionsCellItem
      icon={<PrintIcon sx={{ color: "rgb(53, 87, 236)" }} />}
      label="Toggle Admin"
      component={Link}
      to={`/printPerson/${params.row._id}`}
    // onClick={toggleAdmin(params.id)}
    />,
    edit: <GridActionsCellItem
      icon={<EditIcon sx={{ color: "#FFFF00" }} />}
      label="Toggle Admin"
      component={Link}
    to={`/updatePerson/${params.row._id}`}
    />,
    see: <GridActionsCellItem
    icon={<ZoomInIcon sx={{ color: "#FFFF00" }} />}
    label="Toggle Admin"
    component={Link}
    to={`/persondetail/${params.row._id}`}
  />

  }
  const secont = acction==='see'? actionButtons.see: actionButtons.edit;
  return [actionButtons.print, secont];
}
